...
import { BarChart3, Brain, CalendarDays, CandlestickChart, Feather, LayoutDashboard, LogOut, Monitor, Moon, PieChart, Settings, Sparkles, Star, Sun } from "lucide-react";
...
export function AppShell({ user, children }: { user: User; children: ReactNode }) {
  ...
  return <div className="themed relative z-10 min-h-dvh">
    ...
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] flex-col border-r border-line bg-paper/80 backdrop-blur-xl md:flex">
      ...
      <div className="space-y-3 px-3 pb-5"><Link href="/intelligence/coach" className="group block rounded-xl border border-brand/20 bg-brand-soft/50 p-3 transition-all hover:border-brand/35 hover:bg-brand-soft"><div className="flex items-center gap-2 text-[11px] font-semibold text-brand"><Sparkles className="h-3.5 w-3.5" /> Ask Quill</div><p className="mt-1 text-[10.5px] leading-relaxed text-sub">Review your recorded behaviour with Quill Coach.</p></Link><div className="rounded-xl border border-line bg-card p-3"><div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-medium text-sub"><span className="pulse-dot h-1.5 w-1.5 rounded-full bg-up" /> Live sync on</div><p className="text-[11px] leading-relaxed text-faint">Prices refresh automatically while you write.</p></div><UserChip user={user}/><div className="flex items-center gap-2"><ThemeToggle/><motion.button whileTap={{ scale: .985 }} onClick={signOut} className="flex h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-line text-[13px] font-medium text-sub transition-colors hover:bg-line/50 hover:text-ink cursor-pointer"><LogOut className="h-4 w-4"/> Sign out</motion.button></div></div>
    </aside>
    ...
    <Link href="/intelligence/coach" aria-label="Ask Quill Coach" className="fixed bottom-[5.25rem] right-4 z-40 md:bottom-6 md:right-6"><motion.span whileHover={{ y: -2 }} whileTap={{ scale: .94 }} className="flex h-12 w-12 items-center justify-center rounded-full bg-brandsolid text-brandon shadow-[0_14px_32px_-12px_var(--brand)] ring-1 ring-brand/25"><Sparkles className="h-4 w-4" /></motion.span></Link>
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-paper/90 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl md:hidden"><div className="grid grid-cols-5">{NAV.slice(0,5).map((item) => { const active = isActive(item.href); return <Link key={item.href} href={item.href} className="relative flex min-h-14 min-w-0 flex-col items-center justify-center gap-1 py-2"><span className="sr-only">{item.label}</span>{active && <motion.span layoutId="mnav-pill" className="absolute -top-px h-0.5 w-7 rounded-full bg-brand" transition={{ type:"spring", bounce:.2, duration:.5 }}/>}<motion.span whileTap={{ scale: .78 }} animate={active ? { y: -1 } : { y: 0 }} className="flex"><item.icon className={cn("h-[18px] w-[18px]",active?"text-brand":"text-faint")} strokeWidth={active?2.2:1.9}/></motion.span><span className={cn("truncate max-w-full px-0.5 text-[8.5px] font-medium",active?"text-ink":"text-faint")}>{item.label}</span></Link>; })}</div></nav>
    <InstallApp />
  </div>;
}
