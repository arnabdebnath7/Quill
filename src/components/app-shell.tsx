...
export function AppShell({user,children}:{user:User;children:ReactNode}){
  ...
  return <div className="themed relative z-10 min-h-dvh bg-paper">
    ...
    <header className="sticky top-0 z-40 flex h-[60px] items-center justify-between border-b border-line bg-paper/88 px-4 backdrop-blur-xl md:hidden">
      ...
      <div className="flex items-center gap-2"><div className="hidden items-center gap-1.5 rounded-full border border-line bg-card px-2.5 py-1.5 min-[390px]:flex"><span className="pulse-dot h-1.5 w-1.5 rounded-full bg-up"/><span className="text-[10px] font-medium text-faint">Synced</span></div><ThemeToggle/><button onClick={signOut} aria-label="Sign out" className="flex h-10 w-10 items-center justify-center rounded-xl border border-line text-sub hover:text-ink cursor-pointer"><LogOut className="h-4 w-4"/></button></div>
    </header>
