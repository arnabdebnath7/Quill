...
export async function GET(req: Request) {
  ...
  if (q) {
    conds.push(
      or(ilike(trades.symbol, `%${q}%`), ilike(trades.notes, `%${q}%`), ilike(trades.setup, `%${q}%`))!
    );
  }
...
const createSchema = z.object({
  ...
  entryAt: z.string().datetime().or(z.string().regex(/^\d{4}-\d{2}-\d{2}$/)),
  exitAt: z.string().datetime().or(z.string().regex(/^\d{4}-\d{2}-\d{2}$/)).optional().nullable(),
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Invalid trade", details: String(e) }, { status: 400 });
  }
  ...
  const [row] = await db
    ...
    .values({
      ...
      exitAt: closed ? new Date(body.exitAt ?? new Date()) : null,
