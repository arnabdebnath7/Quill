...
const patchSchema = z.object({
  ...
  market: z.enum(["us", "india", "forex", "crypto", "gold"]).optional(),
  ...
  setup: z.string().max(60).optional().nullable(),
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const merged = { ...current, ...body } as typeof current & z.infer<typeof patchSchema>;
  const exitPrice =
    body.exitPrice === undefined ? current.exitPrice : body.exitPrice == null ? null : String(body.exitPrice);
  const status = exitPrice != null ? "closed" : "open";

  const [row] = await db
    .update(trades)
    .set({
      symbol: merged.symbol ? (merged.symbol as string).toUpperCase() : current.symbol,
      ...
      exitPrice,
      entryAt: body.entryAt ? new Date(body.entryAt) : current.entryAt,
      exitAt:
        body.exitAt !== undefined
          ? body.exitAt == null
            ? null
            : new Date(body.exitAt)
          : exitPrice != null && current.exitAt == null
            ? new Date()
            : current.exitAt,
      fees: body.fees != null ? String(body.fees) : current.fees,
      stopLoss: body.stopLoss === undefined ? current.stopLoss : body.stopLoss == null ? null : String(body.stopLoss),
      target: body.target === undefined ? current.target : body.target == null ? null : String(body.target),
