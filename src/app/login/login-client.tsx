...
export function LoginClient() {
  ...
  const startPhoneVerification = () => {
    if (phone.length === 10 && phase === "idle" && phonePhase === "idle") {
              ...
              {phoneStep === "number" ? (
                ...
                >
                  ...
                  <button
                    onClick={signIn}
                    disabled={busy}
                    className="mt-5 flex h-[48px] w-full items-center justify-center gap-3 rounded-full border border-[#E8E5E0] bg-white px-6 text-[15px] font-[500] text-[#1A1A1A] shadow-[0_2px_8px_rgba(0,0,0,.06)] transition-all hover:border-[#D0CCC6] hover:bg-[#FFFEFB] active:scale-[0.98] disabled:opacity-60"
                  >
                    {busy ? (
                      <motion.span
