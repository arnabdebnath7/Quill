    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-[13px] text-muted-foreground">Patterns hiding in {closed.length} closed trades · USD-normalized</p>
        </div>
        <Link href="/today">
          <Button variant="outline"><HeartPulse className="h-4 w-4" /> Daily check-in</Button>
        </Link>
      </div>

      {/* Life x Trading hero */}
      <Card>
        <div className="border-b border-border bg-primary-soft/40 p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-2xl">
              <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-primary"><Sparkles className="h-3.5 w-3.5" /> Life × Trading</div>
              <h2 className="mt-2 font-display text-[22px] font-semibold tracking-[-0.02em]">Your state is part of the data.</h2>
              <p className="mt-1.5 text-[12.5px] leading-relaxed text-muted-foreground">
                Quill compares your daily sleep, energy and focus check-ins with trade outcomes. These are personal signals, not predictions — the more consistent your check-ins, the more useful they become.
              </p>
            </div>
            <div className="rounded-xl border border-border bg-card px-4 py-3 text-right">
              <div className="font-display text-2xl font-semibold tabular"><AnimatedNumber value={psychology.coverage} format={(n) => `${n.toFixed(0)}%`} /></div>
              <div className="text-[10.5px] text-muted-foreground">trades with a check-in</div>
            </div>
          </div>
        </div>
        <CardContent className="grid gap-4 p-5 sm:grid-cols-3 sm:p-6">
          {[
            ...
            <div key={item.label} className="rounded-xl border border-border bg-muted/40 p-4">
              ...
              <div className="mt-0.5 text-[11.5px] text-muted-foreground">{item.detail}</div>
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
        <Card>
          <CardHeader>
            <SectionTitle icon={Brain} title="Readiness buckets" hint={`${psychology.matched}/${closed.length} matched`} className="mb-0" />
          </CardHeader>
          <CardContent className="space-y-3">
            {psychology.buckets.map((b) => (
              <div key={b.key} className="rounded-xl border border-border bg-muted/40 p-4">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-[13px] font-medium">{b.label}</span>
                  <span className={cn("font-mono text-[13px] font-semibold tabular", b.pnl >= 0 ? "text-up" : "text-down")}>{formatMoney(b.pnl, "USD", { sign: true })}</span>
                </div>
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
                  <span>{b.trades} trades</span>
                  <span>{b.winRate.toFixed(0)}% winners</span>
                  <span>{formatMoney(b.avgPnl, "USD", { sign: true })} avg</span>
                </div>
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-muted">
                  <motion.div className="h-full rounded-full bg-primary" initial={{ width: 0 }} animate={{ width: `${Math.min(100, Math.max(0, b.winRate))}%` }} transition={{ duration: 0.7, ease: EASE }} />
                </div>
              </div>
            ))}
            <div className="rounded-xl bg-primary-soft/60 px-4 py-3 text-[12px] leading-relaxed text-muted-foreground">
              <strong className="font-semibold text-foreground">Read this carefully:</strong> a strong bucket is a signal worth investigating, not proof that your mood causes profits. Keep logging until the pattern survives a bigger sample.
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <SectionTitle icon={HeartPulse} title="Check-in consistency" className="mb-0" />
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl border border-border bg-muted/40 p-4">
                <div className="font-display text-[24px] font-semibold tabular">{checkins?.length ?? 0}</div>
                <div className="mt-0.5 text-[11px] text-muted-foreground">days logged / 90</div>
              </div>
              <div className="rounded-xl border border-border bg-muted/40 p-4">
                <div className="font-display text-[24px] font-semibold tabular">{psychology.checkinDays}</div>
                <div className="mt-0.5 text-[11px] text-muted-foreground">days with a plan</div>
              </div>
            </div>
            <div className="mt-5 flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary-soft text-primary"><Moon className="h-5 w-5" /></div>
              <div>
                <div className="text-[13px] font-semibold">Sleep, energy, focus</div>
                <div className="text-[11.5px] text-muted-foreground">Three variables Quill currently uses for readiness.</div>
              </div>
            </div>
            <div className="mt-5 rounded-xl border border-border bg-muted/40 p-4">
              <div className="flex items-center justify-between text-[12px]">
                <span>Matched trade coverage</span>
                <span className="font-semibold tabular">{psychology.coverage.toFixed(0)}%</span>
              </div>
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-muted">
                <div className="h-full rounded-full bg-primary transition-all duration-700" style={{ width: `${Math.min(100, psychology.coverage)}%` }} />
              </div>
              <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">The goal is not 100% overnight. Consistency is what makes your personal signal trustworthy.</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground"><HeartPulse className="h-3.5 w-3.5 text-primary" /> Mood context</div>
              <h2 className="mt-1.5 font-display text-[18px] font-semibold">Your outcomes by logged mood</h2>
            </div>
            <div className="text-[10.5px] text-muted-foreground">90-day window</div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="grid gap-2.5 sm:grid-cols-5">
            {psychology.moodRows.map((row) => (
              <div key={row.mood} className="rounded-xl border border-border bg-muted/40 p-3.5">
                <div className="text-[12px] font-semibold capitalize">{row.mood}</div>
                <div className={cn("mt-1 font-mono text-[14px] font-semibold tabular", row.pnl >= 0 ? "text-up" : "text-down")}>{row.trades ? formatMoney(row.pnl, "USD", { sign: true }) : "—"}</div>
                <div className="mt-0.5 text-[10.5px] text-muted-foreground">{row.trades ? `${row.trades} trades · ${row.winRate.toFixed(0)}% wins` : "No matched trades"}</div>
              </div>
            ))}
          </div>
          {checkinsLoading && <p className="mt-3 text-[11px] text-muted-foreground">Refreshing check-in context…</p>}
        </CardContent>
      </Card>

      <motion.div variants={staggerContainer(0.06)} initial="initial" animate="animate" className="grid gap-4 sm:grid-cols-2">
        {[
          ...
          <motion.div key={c.label} variants={riseItem}>
            <Card>
              <CardHeader>
                <SectionTitle icon={c.icon} title={c.label} className="mb-0" />
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="font-display text-[19px] font-semibold">{c.item.t.symbol}</div>
                    <div className="mt-0.5 text-[12px] text-muted-foreground">{c.item.t.setup ?? "No setup"} · {format(new Date(c.item.t.entryAt), "MMM d")} · {c.item.t.side}</div>
                    {c.item.t.notes && <p className="mt-2 line-clamp-2 text-[12px] leading-relaxed text-muted-foreground">{c.item.t.notes}</p>}
                  </div>
                  <div className={cn("shrink-0 font-mono text-[18px] font-semibold tabular", c.tone === "up" ? "text-up" : "text-down")}>{formatMoney(c.item.pnl, "USD", { sign: true })}</div>
                </div>
              </CardContent>
            </Card>
          </motion.div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader><SectionTitle icon={PieChart} title="P&L by market" className="mb-0" /></CardHeader>
          <CardContent><HBars data={byMarket} format={(v) => formatMoney(v, "USD", { sign: true })} /></CardContent>
        </Card>
        <Card>
          <CardHeader><SectionTitle icon={CalendarDays} title="P&L by weekday" className="mb-0" /></CardHeader>
          <CardContent>{byWeekday.length ? <HBars data={byWeekday} format={(v) => formatMoney(v, "USD", { sign: true })} /> : <p className="text-[13px] text-muted-foreground">Not enough data.</p>}</CardContent>
        </Card>
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader><SectionTitle icon={PieChart} title="P&L by setup" className="mb-0" /></CardHeader>
          <CardContent><HBars data={bySetup} format={(v) => formatMoney(v, "USD", { sign: true })} /></CardContent>
        </Card>
        <Card>
          <CardHeader><SectionTitle icon={Gauge} title="Long vs short" className="mb-0" /></CardHeader>
          <CardContent>
                ...
                <div key={s.label} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-baseline justify-between">
                      ...
                      <span className={cn("font-mono text-[14px] font-semibold tabular", s.stats.pnl >= 0 ? "text-up" : "text-down")}>{formatMoney(s.stats.pnl, "USD", { sign: true })}</span>
                    </div>
                    <div className="mt-0.5 text-[11.5px] text-muted-foreground">{s.stats.total} trades · {s.stats.rate.toFixed(0)}% winners</div>
            ...
            <p className="mt-5 rounded-xl bg-muted/60 px-3.5 py-2.5 text-[12px] leading-relaxed text-muted-foreground">
