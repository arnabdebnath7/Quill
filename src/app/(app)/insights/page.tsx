...
export default function InsightsPage() {
  ...
  const lifeTradeRows = useMemo(() => closed.map((t) => {
    ...
    return {
      ...
      checkin,
      readiness,
      moodScore: checkin?.mood ? MOOD_SCORE[checkin.mood] ?? 3 : null,
    };
  }), [closed, checkinMap]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-5">
          <div className="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-faint"><Crosshair className="h-3.5 w-3.5 text-brand" /> Expectancy / trade</div>
          <AnimatedNumber value={outline!.expectancy} format={(n) => formatMoney(n, "USD", { sign: true })} className={cn("font-display text-[28px] font-semibold tabular", outline!.expectancy >= 0 ? "text-up" : "text-down")} />
          <p className="mt-2 text-[12px] leading-relaxed text-faint">What an average trade is worth to you. Positive expectancy + discipline is the whole game.</p>
        </Card>
        <Card className="p-5">
          <div className="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-faint"><Scale className="h-3.5 w-3.5 text-brand" /> Payoff ratio</div>
          <div className="font-display text-[28px] font-semibold tabular">{outline!.avgLoss > 0 ? (outline!.avgWin / outline!.avgLoss).toFixed(2) : "∞"}</div>
          <p className="mt-2 text-[12px] leading-relaxed text-faint">Avg win {formatMoney(outline!.avgWin, "USD")} vs avg loss {formatMoney(outline!.avgLoss, "USD")}.</p>
        </Card>
        <Card className="p-5">
          <div className="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-faint"><Brain className="h-3.5 w-3.5 text-brand" /> Win rate</div>
          <AnimatedNumber value={outline!.winRate} format={(n) => `${n.toFixed(1)}%`} className="font-display text-[28px] font-semibold tabular" />
          <p className="mt-2 text-[12px] leading-relaxed text-faint">Win rate alone means nothing — pair it with the payoff ratio.</p>
        </Card>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
        <Card className="p-5 sm:p-6">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div><div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-faint"><Gauge className="h-3.5 w-3.5 text-brand" /> Readiness impact</div><h2 className="mt-1.5 font-display text-[18px] font-semibold">How your starting state behaves</h2></div>
            <div className="rounded-xl bg-line/30 px-2.5 py-1.5 text-[10.5px] text-faint">{psychology.matched}/{closed.length} matched</div>
          </div>
          <div className="space-y-3">
            {psychology.buckets.map((b) => (
              <div key={b.key} className="rounded-2xl border border-line bg-paper p-4">
                <div className="flex items-center justify-between gap-3"><span className="text-[13px] font-medium">{b.label}</span><span className={cn("font-mono text-[13px] font-semibold tabular", b.pnl >= 0 ? "text-up" : "text-down")}>{formatMoney(b.pnl, "USD", { sign: true })}</span></div>
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-faint"><span>{b.trades} trades</span><span>{b.winRate.toFixed(0)}% winners</span><span>{formatMoney(b.avgPnl, "USD", { sign: true })} avg</span></div>
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-line/50"><motion.div className="h-full rounded-full bg-brand" initial={{ width: 0 }} animate={{ width: `${Math.min(100, Math.max(0, b.winRate))}%` }} transition={{ duration: .7, ease: EASE }} /></div>
              </div>
            ))}
          </div>
        ...
        <Card className="p-5 sm:p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-line bg-paper p-4"><div className="font-display text-[24px] font-semibold tabular">{checkins?.length ?? 0}</div><div className="mt-0.5 text-[11px] text-faint">days logged / 90</div></div>
            <div className="rounded-2xl border border-line bg-paper p-4"><div className="font-display text-[24px] font-semibold tabular">{psychology.checkinDays}</div><div className="mt-0.5 text-[11px] text-faint">days with a plan</div></div>
          </div>
      ...
      <Card className="p-5 sm:p-6">
        ...
        <div className="grid gap-2.5 sm:grid-cols-5">
          {psychology.moodRows.map((row) => (
            <div key={row.mood} className="rounded-2xl border border-line bg-paper p-3.5">
              ...
              <div className="mt-0.5 text-[10.5px] text-faint">{row.trades ? `${row.trades} trades · ${row.winRate.toFixed(0)}% wins` : "No matched trades"}</div>
            </div>
          ))}
        </div>
