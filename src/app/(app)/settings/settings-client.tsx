...
import { Download, KeyRound, Monitor, Moon, Sun, Trash2 } from "lucide-react";
...
export function SettingsClient({ user }: { user: User }) {
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <Card className="p-5"><div className="mb-4 text-[11px] font-semibold uppercase tracking-[0.12em] text-faint">Your data</div><div className="grid gap-2.5"><button type="button" onClick={exportData} disabled={exporting} className="flex min-h-11 items-center gap-3 rounded-xl border border-line px-4 py-3 text-left transition-all hover:bg-line/40 active:scale-[0.995] disabled:opacity-60 cursor-pointer"><Download className="h-4 w-4 text-brand" /> <div><div className="text-[13.5px] font-medium">{exporting ? "Preparing backup…" : "Export everything"}</div><div className="text-[11.5px] text-faint">Trades, entries and watchlist as JSON. Your data is yours.</div></div></button><button type="button" onClick={() => setConfirm("clear")} className="flex min-h-11 items-center gap-3 rounded-xl border border-down/30 px-4 py-3 text-left transition-all hover:bg-down-soft active:scale-[0.995] cursor-pointer"><Trash2 className="h-4 w-4 text-down" /><div><div className="text-[13.5px] font-medium text-down">Clear all data</div><div className="text-[11.5px] text-faint">Start completely blank. No undo.</div></div></button></div></Card>
      <Card className="p-5"><div className="mb-4 text-[11px] font-semibold uppercase tracking-[0.12em] text-faint">About</div><div className="space-y-3 text-[13px] leading-relaxed text-sub"><div className="flex items-start gap-2.5"><KeyRound className="mt-0.5 h-4 w-4 shrink-0 text-brand" /><p>Sign-in is protected by Firebase Auth, with credentials verified server-side before a Quill session is created.</p></div><p className="text-[12px] text-faint">Market data: Yahoo Finance (US, NSE, FX & metals) and CoinGecko (crypto) — delay varies by venue. Quill v1.0, built for people who write things down.</p></div></Card>
      <Dialog open={confirm != null} onClose={() => !busy && setConfirm(null)} title="Clear everything?"><p className="text-[13.5px] leading-relaxed text-sub">Every trade and every entry will be permanently deleted. Blank slate.</p><div className="mt-5 flex justify-end gap-2.5"><Button variant="ghost" disabled={busy} onClick={() => setConfirm(null)}>Cancel</Button><Button variant="danger" loading={busy} onClick={runAction}>Delete all</Button></div></Dialog>
    </div>
  );
}
