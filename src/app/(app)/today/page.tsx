...
import { cn, formatMoney, num, tradePnl } from "@/lib/utils";
...
export default function TodayPage() {
  ...
  if (existing && !hydrated) {
    setMood(existing.mood ?? null); setEnergy(existing.energy ?? null); setFocus(existing.focus ?? null); setSleepHours(existing.sleepHours ?? ""); setIntention(existing.intention); setTradingPlan(existing.tradingPlan); setReflection(existing.reflection); setHydrated(true);
  }
  ...
  const readiness = Math.round(((energy ?? 3) + (focus ?? 3) + (sleepHours ? Math.min(5, Math.round(Number(sleepHours) / 2)) : 3)) / 3 / 5 * 100);
    ...
    <section className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
      <Card className="p-5 sm:p-6"><div className="flex items-center gap-2"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-soft text-brand"><Feather className="h-[18px] w-[18px]"/></div><div><h2 className="font-display text-[16px] font-semibold">Check in</h2><p className="text-[11.5px] text-faint">A two-minute snapshot of how you are entering the day.</p></div></div>
        <div className="mt-6 space-y-6">
          ...
          <div><label htmlFor="sleep" className="mb-2 block text-[12px] font-medium text-sub">Sleep last night</label><div className="relative"><Input id="sleep" type="number" min="0" max="24" step="0.5" inputMode="decimal" value={sleepHours} onChange={e => setSleepHours(e.target.value)} placeholder="7.5"/><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-faint">hours</span></div></div>
          <div><label htmlFor="intention" className="mb-2 block text-[12px] font-medium text-sub">Today's intention</label><Textarea id="intention" value={intention} onChange={e => setIntention(e.target.value)} placeholder="What would make today feel well spent?" className="min-h-[88px]"/></div>
          <div className="grid gap-5 sm:grid-cols-2"><Textarea label="Trading plan" value={tradingPlan} onChange={e => setTradingPlan(e.target.value)} placeholder="Setups, markets, max risk, or one rule you will not break." className="min-h-[150px]"/><Textarea label="End-of-day reflection" value={reflection} onChange={e => setReflection(e.target.value)} placeholder="What did you learn? You can fill this tonight." className="min-h-[150px]"/></div>
