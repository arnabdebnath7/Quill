...
function readinessFromCheckin(checkin: DailyCheckin | null) {
  ...
  const parts = [energy, focus, sleepHours == null ? null : Math.min(10, Math.max(0, sleepHours)) * 10 / 8].filter((v): v is number => v != null);
...
function bucketLabel(score: number | null) {
...
export function buildIntelligence(input: {
  ...
  const strongestSetup = [...bySetup.entries()]
    .filter(([, v]) => v.count >= 3)
  ...
  const lowReadinessTrades = closed.filter(({ trade }) => lowReadinessDates.has((trade.entryAt as Date).toISOString().slice(0, 10)));
  ...
  const normalRuleLossRate = rulesTrades.filter(({ trade }) => trade.rulesFollowed === true).length
    ? (rulesTrades.filter(({ trade }) => trade.rulesFollowed === true && x.pnl <= 0).length / rulesTrades.filter(({ trade }) => trade.rulesFollowed === true).length) * 100
    : null;
  ...
  observations.push({ label: "Weekly tape", value: `${recent.length} closed trades`, detail: `${recentPnl >= 0 ? "+" : ""}${recentPnl.toFixed(2)} P&L over the last 7 days`, tone: recentPnl >= 0 ? "positive" : "caution" });
  ...
  if (strongestSetup) {
    const [name, stats] = strongestSetup;
    const expectancy = stats.pnl / stats.count;
    edge.push({ label: "Best documented setup", value: name, detail: `${stats.count} trades · ${expectancy >= 0 ? "+" : ""}${expectancy.toFixed(2)} average P&L · ${pct((stats.wins / stats.count) * 100)} wins`, tone: expectancy > 0 ? "positive" : "neutral" });
  }
  ...
  if (lowReadinessTrades.length >= 3 && lowReadinessLossRate != null) {
    risks.push({ label: "Low-readiness pattern", value: `${pct(lowReadinessLossRate)} loss rate`, detail: `${lowReadinessTrades.length} trades entered on low-readiness days. Treat as a review signal, not a prediction.`, tone: lowReadinessLossRate >= 60 ? "caution" : "neutral" });
  }
  if (ruleBreaks.length >= 2 && ruleBreakLossRate != null) {
    const detail = normalRuleLossRate == null ? `${ruleBreaks.length} rule-break trades logged` : `${pct(ruleBreakLossRate)} loss rate vs ${pct(normalRuleLossRate)} when rules were followed`;
    risks.push({ label: "Rule adherence", value: `${ruleBreaks.length} breaks`, detail, tone: ruleBreakLossRate > (normalRuleLossRate ?? 50) ? "caution" : "neutral" });
  }
  ...
  return {
    ...
    readiness,
    weekly: { pnl: recentPnl, trades: recent.length, winRate, previousPnl, deltaPnl: recent.length && previous.length ? recentPnl - previousPnl : null, journalEntries: input.journals.filter((j) => new Date(j.createdAt) >= recentCutoff).length },
    observations,
    edge,
    risks,
    nextActions,
