...
import type { JournalEntry, Trade, WatchlistItem } from "@/db/schema";
...
export function useCreateTrade() {
  ...
  return useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      api<{ trade: Trade }>("/api/trades", { method: "POST", body: JSON.stringify(payload) }).then((r) => r.trade),
    onMutate: async (payload) => {
      ...
      const optimistic = {
        ...
        entryPrice: String(payload.entryPrice),
        exitPrice: payload.exitPrice != null ? String(payload.exitPrice) : null,
        entryAt: payload.entryAt ? new Date(String(payload.entryAt)) : new Date(),
...
export function useCreateEntry() {
  ...
  return useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      api<{ entry: JournalEntry }>("/api/journal", { method: "POST", body: JSON.stringify(payload) }).then((r) => r.entry),
    onMutate: async (payload) => {
...
export function useUpdateEntry() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, ...payload }) => {
      ...
      snapshots.forEach(([key, data]) => {
        if (!data) return;
        qc.setQueryData(key, data.map((e) => (e.id === id ? ({ ...e, ...payload } as JournalEntry) : e)));
      });
      return { snapshots };
    },
    onError: (e, _v, ctx) => {
...
export function useWatchlist() {
  return useQuery({
    queryKey: ["watchlist"],
    queryFn: () => api<{ items: WatchlistItem[] }>("/api/watchlist").then((r) => r.items),
  });
}
